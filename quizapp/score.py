...
def oil_dry_anls(test_code):
    print(test_code)
    skin = QuizModal.objects.filter(Q(choice__question__category__code=100) & Q(skin_test__code=test_code))
    ...
    if strip_score:
        if 7 <= strip_score <= 8:
            strip_analysis = OIL_DRY_STATEMENT[0][1]
        elif 5 <= strip_score <= 6:
            strip_analysis = OIL_DRY_STATEMENT[1][1]
        elif 3 <= strip_score <= 4:
            strip_analysis = OIL_DRY_STATEMENT[2][1]
        elif 1 <= strip_score <= 2:
            strip_analysis = OIL_DRY_STATEMENT[3][1]
    
    quiz_score = skin.filter(~Q(choice__question__code__in=[100,101])).aggregate(Sum('choice__marks'))['choice__marks__sum'] or 0.00
    quiz_analysis = ''
    if quiz_score:
        if 16 <= quiz_score <= 20:
            quiz_analysis = OIL_DRY_STATEMENT[0][1]
        elif 12 <= quiz_score <= 15:
            quiz_analysis = OIL_DRY_STATEMENT[1][1]
        elif 8 <= quiz_score <= 11:
            quiz_analysis = OIL_DRY_STATEMENT[2][1]
        elif 5 <= quiz_score <= 7:
            quiz_analysis = OIL_DRY_STATEMENT[3][1]
    
    final_output = ''
    if strip_analysis == quiz_analysis:
        final_output = strip_analysis
    #Oily Skin
    elif strip_analysis == OIL_DRY_STATEMENT[0][1] and quiz_analysis == OIL_DRY_STATEMENT[1][1]:
        final_output = OIL_DRY_STATEMENT[0][1] #Oily Skin
    elif strip_analysis == OIL_DRY_STATEMENT[0][1] and quiz_analysis == OIL_DRY_STATEMENT[2][1]:
        final_output = OIL_DRY_STATEMENT[2][1] #Normal Skin
    elif strip_analysis == OIL_DRY_STATEMENT[0][1] and quiz_analysis == OIL_DRY_STATEMENT[3][1]:
        final_output = OIL_DRY_STATEMENT[3][1]
    
    #Combination Skin
    elif strip_analysis == OIL_DRY_STATEMENT[1][1] and quiz_analysis == OIL_DRY_STATEMENT[0][1]:
        final_output = OIL_DRY_STATEMENT[1][1] #Combination Skin
    elif strip_analysis == OIL_DRY_STATEMENT[1][1] and quiz_analysis == OIL_DRY_STATEMENT[2][1]:
        final_output = OIL_DRY_STATEMENT[1][1] #Combination Skin
    elif strip_analysis == OIL_DRY_STATEMENT[1][1] and quiz_analysis == OIL_DRY_STATEMENT[3][1]:
        final_output = OIL_DRY_STATEMENT[3][1] #Dry Skin
    
     #Normal Skin
    elif strip_analysis == OIL_DRY_STATEMENT[2][1] and quiz_analysis == OIL_DRY_STATEMENT[0][1]:
        final_output = OIL_DRY_STATEMENT[2][1] #Normal Skin
    elif strip_analysis == OIL_DRY_STATEMENT[2][1] and quiz_analysis == OIL_DRY_STATEMENT[1][1]:
        final_output = OIL_DRY_STATEMENT[1][1] #Combination Skin
    elif strip_analysis == OIL_DRY_STATEMENT[2][1] and quiz_analysis == OIL_DRY_STATEMENT[3][1]:
        final_output = OIL_DRY_STATEMENT[2][1] #Normal Skin
    
    #Dry Skin
    elif strip_analysis == OIL_DRY_STATEMENT[3][1] and quiz_analysis == OIL_DRY_STATEMENT[0][1]:
        final_output = OIL_DRY_STATEMENT[3][1] #Dry Skin
    elif strip_analysis == OIL_DRY_STATEMENT[3][1] and quiz_analysis == OIL_DRY_STATEMENT[1][1]:
        final_output = OIL_DRY_STATEMENT[3][1] #Dry Skin
    elif strip_analysis == OIL_DRY_STATEMENT[3][1] and quiz_analysis == OIL_DRY_STATEMENT[3][1]:
        final_output = OIL_DRY_STATEMENT[2][1] #Normal Skin

    try:
        hydration = Hydration.objects.get(skin_test__code=test_code)
...
def sen_res_anls(test_code):
    skin = QuizModal.objects.filter(Q(choice__question__category__code=102) & Q(skin_test__code=test_code))
...
def acne_anls(test_code):
    skin = QuizModal.objects.filter(Q(choice__question__category__code=103) & Q(skin_test__code=test_code))
...
def pigmentation_anls(test_code):
    skin = QuizModal.objects.filter(Q(choice__question__category__code=107) & Q(skin_test__code=test_code))
...
def ageing_anls(test_code):
    skin = QuizModal.objects.filter(Q(choice__question__category__code=108) & Q(skin_test__code=test_code))
    ageing_score = skin.aggregate(Sum('choice__marks'))['choice__marks__sum'] or 0.00
    ageing_analysis = ''
    if ageing_score:
        if 6 <= ageing_score <= 9:
            ageing_analysis = WRINKLETIGHT_STATEMENT[0][1]
        elif 10 <= ageing_score <= 14:
            ageing_analysis = WRINKLETIGHT_STATEMENT[1][1]
        elif 15 <= ageing_score <= 24:
            ageing_analysis = WRINKLETIGHT_STATEMENT[2][1]
    
    context = {
        'ageing_anls_score': ageing_score,
        'ageing_anls_analysis' : ageing_analysis
    }
    return context

def product_info(test_code, concern_obj, oil_dry_ctx, sen_res_ctx, acne_anls_ctx):
    pro_code = 0    
